...
### 3. 내레이션 (narration)
- [ ] 인사말 없이 바로 본문 시작
- [ ] 도입 → 본문 → 전환 → 결론 흐름이 구조적으로 보임
- [ ] 필요 시 `번호 + 소제목` 형식으로 섹션이 명확히 구분됨
- [ ] 섹션이 바뀔 때 문단이 나뉘어 화면에서도 읽기 쉬움
- [ ] 섹션 전환 시 연결어 사용
- [ ] 최소 1000자 이상
...
- 기대: narration도 필요 시 번호+소제목 형식과 줄바꿈으로 구조가 선명하게 보임.
...
- **내레이션**: 도입/본문/전환 구조가 보이고, 필요 시 1번, 2번 같은 번호+소제목 형식과 문단 나눔이 살아 있는지 확인.
