...
import { detectNewBoardChipIds } from '../../utils/eduMobileBoardStripPopIn'
...
export default function EduMobileBoardStrip({ board, hidden = false, onChipTap }: Props) {
  ...
  const newcomers = detectNewBoardChipIds(prevFilledRef.current, filledIds)
    ...
    >
      <div className="px-3 py-2">
        {sorted.length === 0 ? (
          ...
          <LayoutGroup id="edu-mobile-board-strip">
            <div className="flex flex-wrap gap-1.5">
              {filled.map(slot => (
                <motion.button
                  ...
                  type="button"
                  layout
                  layoutId={`strip-chip-${slot.layer_id}`}
              ...
              <AnimatePresence mode="popLayout">
                {current ? (
                  <motion.button
                    ...
                    type="button"
                    layout
                    layoutId={`strip-chip-${current.layer_id}`}
                    initial={{ scale: 0.95, opacity: 0.72 }}
                    animate={{ scale: 1, opacity: 1 }}
