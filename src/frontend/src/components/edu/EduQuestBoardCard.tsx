...
import EduQuestDifficultyBadge from './EduQuestDifficultyBadge'
    ...
    >
      ...
      <div className={size === 'featured' ? 'p-4 space-y-2' : 'p-3 space-y-1.5'}>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <EduQuestDifficultyBadge
            quest={quest}
            size={size === 'featured' ? 'md' : 'sm'}
            showHint={quest.difficulty_level === 1}
          />
          {showRecommended && quest.recommended_for_you && (
            <span
              className="px-2 py-0.5 rounded-full font-medium"
              style={{
                fontSize: '0.65rem',
