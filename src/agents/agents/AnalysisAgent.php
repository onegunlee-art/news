...
[제목/저자]
- URL 슬러그 또는 본문/메타에서 original_title, author를 추출. 없으면 기사 제목을 original_title로 사용.
...

{
  ...
  "original_title": "영문 원제 (본문과 일치하면 그대로 사용)",
  "content_summary": "가독성 좋은 구조화 형식. 한글 제목 (영문 제목)\n\n[논리적 섹션별 요약 - 영문 1줄 → 한글 1줄 교차]. 소제목이 있으면 섹션 제목으로 사용, 없으면 도입·전개·데이터·결론 등으로 구분. 최소 600자 이상.",
  "key_points": [
  ...
  "narration": "지스터(The Gist 독자)를 위한 내레이션. 인사말 없이 바로 본문으로 시작. 도입 → 주요 내용(수치·배경·결론)을 자연스럽게 이어서 작성. 지스터가 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 반드시 최소 1000자 이상."
}
PROMPT;
    }
...
- 기사가 중간에 잘렸거나 광고로 끊긴 것처럼 보이면, 읽을 수 있는 부분까지만 분석하고 narration 끝에 [기사 일부만 분석됨]이라고만 표시하세요. 나머지 필드는 정상 작성.
...
[제목/저자]
- URL 슬러그 또는 본문에서 original_title, author 추출.
...

{
  ...
  "narration": "지스터를 위한 내레이션. 인사말 없이 바로 본문으로. 도입 → 주요 내용 자연스럽게. 귀로만 들어도 이해 가능하도록 상세히. 반드시 최소 1000자 이상. (일부만 분석된 경우 끝에 [기사 일부만 분석됨] 표시)"
}
PROMPT;
    }

    /**
     * 기본(Foreign Affairs 등): 소제목 기반 content_summary, pull-quote 무시, narration 1000자+
     */
...

{
  ...
  "content_summary": "줄글이 아닌 가독성 좋은 구조화 형식으로 작성. 참조 이미지 5번(가독성 형식)을 반드시 확인하세요.\n\n한글 제목 (영문 제목)\n한글 부제 (영문 부제)\n\n[첫 번째 소제목 나오기 전까지의 전체 요약 - 영문 1줄 → 한글 1줄 교차, 짧은 문단]\n예:\nThe article argues that Washington must study Ukraine's innovations.\n이 기사는 워싱턴이 우크라이나의 혁신을 연구해야 한다고 주장한다.\n\n소제목1 한글 (영문 소제목)\n[해당 섹션 - 영문 1줄 → 한글 1줄 교차, 문단 구분]\n\n소제목2 한글 (영문 소제목)\n[해당 섹션 - 영문 1줄 → 한글 1줄 교차]\n\n... (소제목이 더 있으면 반복)\n\n- 연속된 줄글 금지. 섹션별 제목·여백·문단 구분으로 가독성 확보\n- 소제목: 기사 본문 중간에 큰 글씨/대문자로 된 섹션 헤딩\n- 소제목이 없으면 '한글 제목/부제/전체 요약(영한 교차)'만 작성\n- 최소 600자 이상",
  "key_points": [
  ...
  "narration": "지스터(The Gist 독자)를 위한 내레이션. '지스터 여러분', '시청자 여러분' 등 인사말 없이 바로 본문으로 자연스럽게 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. pull-quote 볼드 텍스트는 포함하지 마세요. 지스터가 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 1000자 이상."
}
PROMPT;
    ...
    /** 인사말 제거: 지스터 여러분, 시청자 여러분 등으로 시작하면 제거하고 자연스럽게 정리 */
...

{
  ...
  "narration": "지스터를 위한 개선된 내레이션. '지스터 여러분', '시청자 여러분' 등 인사말 없이 바로 본문으로 자연스럽게 시작. 최소 900자 이상."
}
PROMPT;
