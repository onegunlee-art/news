...
class AnalysisAgent extends BaseAgent
{
    ...
    private function performFullAnalysis(ArticleData $article): AnalysisResult
    {
        ...
        if (!empty($imageUrls)) {
            ...
            $prompt = "[참조 이미지 - 반드시 확인] 첨부된 참조 이미지를 순서대로 확인하세요.\n\n"
                . "1) 제목·부제목: Foreign Affairs 기사 상단 스크린샷. 제목(가장 큰 볼드체)과 부제목(이탤릭체) 위치를 확인하고, 부제목을 추출한 뒤 news_title과 같은 방식으로 한글 직역하여 subtitle 필드에 넣으세요.\n"
    ...
    private function buildFullAnalysisPrompt(ArticleData $article): string
    {
        ...
        $subtitleBlock = ($scrapedSubtitle !== null && $scrapedSubtitle !== '')
            ? "\n기사 부제목(HTML에서 추출됨 - 이 영문을 news_title과 같은 방식으로 한글 직역하여 subtitle 필드에 넣으세요): " . $scrapedSubtitle
            : '';
...
[부제목(Subtitle) 추출 및 한글 직역 방법]
- 1단계: 영문 부제목 추출. 위에 "기사 부제목(HTML에서 추출됨)"이 있으면 그 값을 사용. 없으면 본문에서 메인 제목 바로 아래 부제목(짧은 이탤릭체 문장)을 찾아 추출.
- 2단계: 추출한 영문 부제목을 news_title과 동일한 방식으로 한글 직역. 예: "Will Washington Repeat Moscow's Mistakes?" → "워싱턴이 모스크바의 실수를 반복할 것인가?"
- subtitle 필드에는 한글 직역 결과를 넣으세요. 부제목이 없으면 빈 문자열("").
...

{
  ...
  "subtitle": "부제목의 한글 직역. news_title과 동일한 방식(단어 그대로 직역). 없으면 빈 문자열.",
  ...
  "narration": "지스터(The Gist 독자)를 위한 내레이션. '지스터 여러분', '시청자 여러분' 등 인사말 없이 바로 본문으로 자연스럽게 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. pull-quote 볼드 텍스트는 포함하지 마세요. 지스터가 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 900자 이상."
}
PROMPT;
     ...
     * - title_subtitle.jpg / subtitle_foreign_affairs.png: 제목·부제목 추출
