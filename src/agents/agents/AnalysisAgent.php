...

{
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 시작. 도입 → 주요 내용(수치·배경·결론)을 자연스럽게 이어서 작성. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 반드시 최소 900자 이상."
}
PROMPT;
    }
...

{
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 900자 이상."
}
PROMPT;
    }
...

{
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 자연스럽게 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. pull-quote 볼드 텍스트는 포함하지 마세요. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 900자 이상."
}
PROMPT;
