...

{
  ...
  "content_summary": "가독성 좋은 구조화 형식. 한글 제목 (영문 제목)\n\n[논리적 섹션별 요약 - 영문 1줄 → 한글 1줄 교차]. 소제목이 있으면 섹션 제목으로 사용, 없으면 도입·전개·데이터·결론 등으로 구분. 의미 단위가 바뀔 때마다 문단 사이에 빈 줄(\\n\\n)을 넣어 가독성을 확보하세요. 최소 600자 이상.",
  "key_points": [
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 시작. 도입 → 주요 내용(수치·배경·결론)을 자연스럽게 이어서 작성. 의미 흐름이 바뀔 때마다 문단을 나누고, 문단과 문단 사이에는 반드시 한 줄을 비워 가독성을 확보하세요. 한 문단에 너무 많은 논점을 몰아넣지 말고 1~3문장 단위로 끊어 쓰세요. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 반드시 최소 900자 이상."
}
PROMPT;
    }
...

{
  ...
  "author": "저자 (없으면 빈 문자열)",
  "original_title": "위 '기사 제목'과 동일하게 그대로 (재구성 금지)",
  "content_summary": "구조화 형식. 한글 제목 (영문 제목)\n\n[섹션별 요약 - 영문 1줄 → 한글 1줄 교차]. 소제목이 있으면 섹션 제목으로 사용. 의미 단위가 바뀔 때마다 문단 사이에 빈 줄(\\n\\n)을 넣어 가독성을 확보하세요. 최소 600자 이상.",
  "key_points": [
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. 의미 흐름이 바뀔 때마다 문단을 나누고, 문단과 문단 사이에는 반드시 한 줄을 비워 가독성을 확보하세요. 한 문단에 너무 많은 논점을 몰아넣지 말고 1~3문장 단위로 끊어 쓰세요. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 900자 이상."
}
PROMPT;
    }
...

{
  ...
  "narration": "독자를 위한 내레이션. 인사말 없이 바로 본문으로 자연스럽게 시작. 도입(무슨 일이 있었는지) → 주요 내용(핵심 사실들)을 자연스럽게 이어서 말하듯 작성. pull-quote 볼드 텍스트는 포함하지 마세요. 의미 흐름이 바뀔 때마다 문단을 나누고, 문단과 문단 사이에는 반드시 한 줄을 비워 가독성을 확보하세요. 한 문단에 너무 많은 논점을 몰아넣지 말고 1~3문장 단위로 끊어 쓰세요. 귀로만 들어도 기사 전체를 이해할 수 있도록 충분히 상세하게. 최소 900자 이상."
}
PROMPT;
