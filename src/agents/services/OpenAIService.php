    ...
    private function generateMockJsonResponse(string $prompt): string
    {
        ...
        if (strpos($prompt, 'key_points') !== false || strpos($prompt, 'narration') !== false || strpos($prompt, 'news_title') !== false || strpos($prompt, 'content_summary') !== false || strpos($prompt, 'translation_summary') !== false) {
            ...
            return json_encode([
                ...
                'original_title' => 'The Limits of Global Economic Policy Shift',
...
조건:
- topic_label: 이 글의 주제를 한국어 25자 이내로, 주체와 행동이 드러나게 작성
  좋은 예: "트럼프의 대중국 관세 145% 부과 결정"
  좋은 예: "한국 반도체 수출 규제 완화 움직임"
  나쁜 예: "미국 경제 정책" (너무 모호)
  나쁜 예: "관세" (단어만)
...
PROMPT;
    ...
    public function createImage(string $prompt, array $options = []): ?string
    {
        ...
        if ($curlError !== '') {
            ...
            error_log('[GPT-Image] ' . $this->lastError . ' | timeout=' . $timeout . 's');
            return null;
        }
        ...
        if ($httpCode !== 200) {
            ...
            error_log('[GPT-Image] API error: ' . $this->lastError);
            return null;
        }
