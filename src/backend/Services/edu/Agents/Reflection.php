...
class Reflection
{
    ...
    public function summarize(
        ...
        if ($isConvergent && $axisLabel !== null) {
            $systemPrompt = <<<PROMPT
학생의 토론 과정을 3줄로 정리해줘.
...
MSG;
        ...
        if (!empty($response['error'])) {
            $fallback = $this->fallbackSummaryLines($initialStance, $stanceChanged, $axisLabel);
            return [
                'success' => false,
        ...
        if (empty($lines)) {
            $lines = $this->fallbackSummaryLines($initialStance, $stanceChanged, $axisLabel);
        }
    ...
    /** @return list<string> */
    private function fallbackSummaryLines(string $initialStance, bool $stanceChanged, ?string $axisLabel = null): array
    {
