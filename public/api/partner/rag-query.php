<?php
/**
 * Partner RAG Query API — 외부 서비스 연동용 (Read-Only)
 *
...
if (!$openai->isConfigured() || !$supabase->isConfigured()) {
    sendError('OpenAI or Supabase not configured', 503);
}

$raw = file_get_contents('php://input');
$input = json_decode($raw, true);
if (!is_array($input) || empty(trim((string) ($input['query'] ?? '')))) {
    sendError('query is required');
}

$query = trim((string) $input['query']);
$limit = max(1, min(50, (int) ($input['limit'] ?? 10)));
$includeAnalysis = (bool) ($input['include_analysis'] ?? false);
$analysisClusterName = trim((string) ($input['analysis_cluster_name'] ?? ''));
...
try {
    ...
    error_log('[partner/rag-query] embedding error: ' . $e->getMessage());
    sendError('Embedding generation failed', 500);
}
if (empty($embedding)) {
    sendError('Empty embedding returned', 500);
}
...
// ============================================================
// 3. Fetch article metadata from MySQL (SELECT only)
// ============================================================
...
if (!empty($newsIds)) {
    try {
        ...
        error_log('[partner/rag-query] MySQL error: ' . $e->getMessage());
    }
}
...
PROMPT;
    ...
    try {
        ...
        error_log('[partner/rag-query] cluster GPT error: ' . $e->getMessage());
    }
}
